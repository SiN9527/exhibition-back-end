...
public class JwtAuthLoginServiceImpl implements JwtAuthLoginService {
    ...
    public JwtAuthLoginServiceImpl(JwtUtil jwtUtil,
                                   JwtService jwtService, @Qualifier("jwtAdminDetailsService")  JwtAdminDetailsService adminDetailsService,
    ...
    public  String memberAuthLogin(MemberLoginRequest loginRequest, HttpServletResponse response) {
        ...
        List<String> roles = userDetails.getAuthorities().stream()
                ...
                .toList();
        String accessToken = jwtService.generateAccessToken(email,member.getMemberId(), type, roles);
    ...
    @Override
    public  String adminAuthLogin(AdminLoginRequest loginRequest, HttpServletResponse response) {
        ...
        if (!isAdmin) {
           throw new AuthenticationFailedException("admin not found");
        }
        UserDetails userDetails;
        String type;
        String storedEncryptedPassword;
        ...
        List<String> roles = userDetails.getAuthorities().stream()
                ...
                .toList();
        logger.info("login account: {}", account);
        String accessToken = jwtService.generateAccessToken(user.getEmail(), user.getAccount(), type, roles);
